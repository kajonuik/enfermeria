...
Decana: Dra. Susana Alvarado Alfaro (2024).
Historia: Creada oficialmente el 31 de octubre de 1984.
Malla Curricular: 10 ciclos. Ciclo 1: Biología, Ciclo 10: Tesis.
Especialidades: Cuidados Intensivos Pediátricos, Salud Familiar.
Instrucciones: Responde de forma breve y amable. Si no sabes algo, invita a ir a la Ciudad Universitaria.
`;
...
async function askAI() {
    ...
    chatBox.innerHTML += `<div id="${tempId}" style="background: #eee; padding: 10px; border-radius: 10px; align-self: flex-start; font-style: italic; color: #777;">Consultando datos...</div>`;
    chatBox.scrollTop = chatBox.scrollHeight;
    ...
    try {
        ...
        const loadingElement = document.getElementById(tempId);
        if (loadingElement) loadingElement.remove();
        chatBox.innerHTML += `<div style="color: #dc3545; font-size: 11px; text-align: center; padding: 5px;">Error de respuesta. Inténtalo de nuevo.</div>`;
    }
    chatBox.scrollTop = chatBox.scrollHeight;
}
